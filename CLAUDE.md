...
- EDGE BUILD PROBE (22.09): стара й нова версія edge-функції ззовні відповідали
  однаково, тож «передеплой зроблено?» не перевірявся без власниці. Тепер функція
  пакета першим рядком обробника відповідає на `GET …/functions/v1/<fn>?version`
  міткою `{fn, build}` із `_shared/build.ts` (без даних, до перевірки доступу).
  Кожен пакет, що міняє edge, ПІДІЙМАЄ `EDGE_BUILD`; агент звіряє мітку сам у
  браузерній панелі. Фронтенд звіряється так само сам — за рядками з нових
  чанків у JS живого сайту (Publish одразу після пушу може викласти СТАРУ збірку,
  якщо Lovable ще не підтягнув коміт — 22.09 так і сталось). Стереже
  `edge-build-probe.test.ts`.
...
- Етап D (`20260907130000_hub_scope_assert`) — чиста перевірка ЖИВОЇ бази:
  падає списком manager-армів без школи у pg_policies / pg_proc / pg_views.
  «Арм» = справжня `has_role(…, 'manager')`, не згадка слова; тригери поза
  перевіркою. Її можна перезапускати; після будь-якої нової manager-політики
  чи RPC — прогнати знову. Те, що вона знайшла на репліці, закрито
  доповненням `20260907125000` (звернення → суперадмін; групові RPC; предикати
  груп; `create_notification`: менеджер сповіщає лише свою школу, менеджера —
  лише його школа, суперадміна — будь-хто; `notify_managers` для незалежних
  іде до `platform_admins`).
- `has_role(<інший користувач>, role)` НЕ є опорою для дозволу: у проді вона
  працює лише з 20260602 (перф-міграція), а версія 20260422 повертала false для
  будь-кого, крім auth.uid(). Роль ЦІЛЬОВОГО користувача читай прямо з
  `user_roles` / `hub_managers` / `platform_admins` у DEFINER-функції.
...
- **11.09, повторний аудит по кожному екрану/формі/ролі — ЛЕНДІНГ і ФОРМИ.**
  Лендінг має ВЛАСНУ палітру в `.landing-root` (`--l-accent` тощо), тож хвиля
  контрасту в index.css його не торкнулась — і він лишався найгіршою сторінкою
  застосунку: білий напис на кнопці 2.41:1, акцент як текст 2.23:1,
  плейсхолдери калькулятора 2.53:1. Тепер там та сама логіка: `--l-accent`
  лишається брендовою ЗАЛИВКОЮ, а `--l-accent-btn` (білий напис 4.97:1) і
  `--l-accent-text` (5.85:1) несуть усе читабельне; `--l-muted`/`--muted2`/
  `--l-warning-text`/`--l-success-text` — так само. Ховер кнопок більше не
  міняє колір (лишився підйом + тінь), інакше напис падав би назад у 2.4:1.
  Стереже блок «лендінг — власна палітра» в `contrast-gate.test.ts`.
  Форми (робот відкрив кожну по кожній ролі): чипи предметів 31px → 40,
  поле власного предмета 14px → 15px (14px змушує iOS ЗУМИТИ всю форму),
  перемикач режиму в ГРОШОВІЙ формі 32 → 44, вимикач дня 24 → 44 зони дотику
  (вигляд 43×24 збережено), поле майстра груп тепер `alignSelf: stretch` —
  воно виглядало на 64px, а дотик ловив лише свій рядок у 30px.
  **Пастка СТЕНДА, що ховала перевірку:** `demo/fake` вважав «видом» усе, для
  чого `viewOf` повертав новий масив — тобто і справжні таблиці зі скоупом.
  Запис у `lessons` мовчки падав із 42809, і стенд НІКОЛИ не проходив
  створення уроку до кінця. Тепер список видів явний (`REAL_VIEWS`).
...
### Матеріали та історія учня (12.09, питання власниці) — do not "simplify"
- Картка в `StudentMaterials` показує дату **І ЧАС** і статус-чип. Причина не
  косметична: власниця спитала «наступний урок сьогодні о 12:30, а домашку
  показує ніби з учорашнього — це я наплутала?». Ні: домашку нормально писати
  НАПЕРЕД, і тоді майбутній урок стоїть першим у списку — але картка показувала
  лише дату, без часу й без ознаки «вже було / ще буде», тож виглядала як запис
  про минуле. Ніколи не прибирати час або чип заради компактності.
- Стан рахує ЧИСТА `lessonStateOf(status, startsAt)`: cancelled → «Скасовано»,
  completed → «Проведено», майбутній scheduled → «Заплановано», **минулий і досі
  scheduled → «Не позначено»** (це і є урок, що чекає відмітки). Статус у базі
  важливіший за час: проведений майбутній лишається проведеним.
- Дві вкладки: «Матеріали» = лише уроки, де щось Є; «Історія» = УСІ уроки з
  оплатою. Історія НЕ фільтрує по вмісту — інакше вона перестає бути журналом.
- Згортання дворівневе: картка складається в ОДИН рядок (найсвіжіша відкрита,
  решта згорнуті + «Розгорнути/Згорнути всі»), а довгі конспект, домашка й
  нотатка всередині мають власне «показати повністю ↔ згорнути». Домашка
  раніше не згорталась узагалі й займала екран — це була скарга власниці.
- Гроші в «Історії» — лише коли ціна СПРАВДІ є (інваріант «відсутнє рендериться
  як відсутнє»). Перенесені борги (`carried_over`) в обидві вкладки не входять.
- Стереже блок «матеріали: коли саме був урок і чи він уже був» в a11y-ratchet.

### Груповий урок збирається у формі уроку (12.09, скарга власниці) — do not revert
- Скарга дослівно: «у груповому уроці не виводить список усіх моїх учнів і я можу
  обрати лише одного, додавати теж не можу». Список був не винен: перемикач
  «👥 Груповий» рендерився під умовою `groups.length > 0`, тобто репетиторка, яка
  ЖОДНОЇ групи ще не має, бачила в формі уроку звичайний список учнів із вибором
  рівно одного — і ніякого натяку, що груповий урок узагалі можливий. Зібрати
  групу можна було тільки так: піти в «Групи» → майстер → додати учасників →
  повернутись у розклад. У самій формі про це не було ні слова.
- Тепер: перемикач видно ЗАВЖДИ (крім хабового — групи школи заводить менеджер);
  у режимі «Груповий» є «➕ Нова група» з МНОЖИННИМ вибором учнів, полем назви і
  «+ Додати учня»; якщо груп ще немає, збирання відкривається одразу, без зайвого
  дотику. Після створення група одразу обрана — урок ставиться тим самим дотиком.
- Запис групи — ОДИН канон: `src/lib/groups.ts` → `createGroupWithStudents()`.
  Ним користуються І майстер на `GroupsPage`, І форма уроку. Другої копії
  `insert into lesson_groups` у застосунку бути не може.
- **ЦІНА в групі свідомо лишається NULL.** Індивідуальна ставка учня — ІНША
  величина (групова зазвичай нижча), і підставити її означало б тихо виставити
  неправильну суму. Інваріант «відсутнє рендериться як відсутнє» діє і тут: ціну
  ставлять на пігулці учасника в «Групах», а до того урок чесно потрапляє в
  задачу «уроки без ціни».
- Стереже блок «груповий урок збирається у самій формі» в a11y-ratchet
  (перемикач без умови на groups.length · множинний вибір · один канон запису ·
  ціна не підставляється · авто-відкриття без груп).

### Відеоінструкція для репетиторів (12.09) — як її перезняти
- Замовлення власниці: екранна інструкція для САМОСТІЙНИХ репетиторів, мобілка,
  з підписами, стрілками й підсвіткою «що за чим натискати». Рішення: 6 коротких
  кліпів (Мій день · поставити урок · груповий урок · Мої учні · домашка й конспект ·
  гроші) + склейка з них у повну інструкцію; без голосу, бо телефон дивляться без
  звуку, плюс окремий текст із таймкодами, щоб власниця начитала своїм голосом.
- Знімалось на стенді аудиторки (`demo/`, некомічений) роботом Playwright:
  контекст 390×844 @2 (`recordVideo` 780×1688), персона `?persona=ind`, згода на
  куки ставиться в `localStorage` ДО показу, щоб банер не ліз у кадр. Шар підказок
  (підпис · кільце · затемнення навколо · стрілка · дотик) інжектиться
  `addInitScript` і живе в `<html>` ПОЗА React-коренем. Готове — mp4 H.264 yuv420p,
  ~1 МБ на кліп, без звукової доріжки.
- **Дві пастки, на яких це ламалось — не повторювати:**
  (1) ініт-скрипт біжить ДО того, як існує `<html>`, тож усе, що чіпляється до DOM,
  мусить чекати `DOMContentLoaded` — інакше `window.__vid` мовчки не створюється і
  підказок у кадрі немає;
  (2) координати цілі треба міряти ПІСЛЯ `scrollIntoViewIfNeeded`. Клік сам гортає
  сторінку, тож заміряне до прокрутки кільце лишається світити в порожнє місце,
  поки підпис розповідає про інше. Саме так вийшов брак у кліпі про групу.
- Зйомка — це ще й аудит: робот проходить справжні шляхи й показує те, чого не
  видно на скріншотах. Так знайдено дефект, полагоджений у `a136eba7` (кнопка
  збирання групи стояла під згином).
- Файли віддано в `~/Desktop/oTutorHub-відео/`. Якщо інтерфейс на цих екранах
  змінився — кліп треба перезняти, інакше інструкція вчить неіснуючого.
...
- `LessonCard.tsx` — його ДИЗАЙН і поведінка недоторкані (used across
  ...
  ⚠️ ВИНЯТОК, рішення власниці 11.09: імʼя учня на картці — ПОСИЛАННЯ в його
  матеріали. Не «відновлювати» звичайний `<div>` з титулом. Адресу дає проп
  `studentHref`, який рахує СТОРІНКА (Dashboard/Schedule), бо тільки вона знає
  тип репетитора: у ХАБОВОГО сторінки учня немає взагалі — `/my-students`
  відкидає його на дашборд, тож там проп = null, як і в учня та на групах.
