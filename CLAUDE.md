...
- EDGE BUILD PROBE (22.09): стара й нова версія edge-функції ззовні відповідали
  ...
  Мітка — авто-штамп вмісту функцій (`_shared/version.ts`, `npm run stamp`;
  ворота `stamp-edge` падають без перештампування) — НЕ ручне число. Функція
  `version` віддає той самий штамп за весь пакет. Фронтенд несе хеш джерел у
  `/version.json` (Publish одразу після пушу може викласти СТАРУ збірку, якщо
