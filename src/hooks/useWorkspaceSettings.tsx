...
export function useWorkspaceSettings() {
  ...
  const { data, isPending, error } = useQuery<WorkspaceQueryData>({
    queryKey,
    enabled,
    /* 26.09 — саме цей запит визначає ПЕРСОНУ, а без персони «Фінанси»
       показують «Не вдалося завантажити» (гілка workspaceUnknown). Спільний
       retry:1 із App.tsx давав дві спроби: на ліміті запитів по IP (мобільний
       оператор, шкільний Wi-Fi — скан логів 16.09) цього мало, і хабовий
       репетитор бачив помилку замість своїх виплат. Читання ідемпотентне,
       тож повторюємо чотири рази з бекофом; у гіршому разі людина чекає
       ~4 секунди замість того, щоб побачити зламаний екран. */
    retry: 3,
