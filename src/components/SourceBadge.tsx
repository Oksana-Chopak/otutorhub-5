...
export function SourceBadge({ source, className, showIcon = true }: Props) {
  if (source === "hub") {
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1 rounded-md bg-warning/15 px-1.5 py-0.5 text-[13px] font-medium text-warning",
          className
        )}
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-md bg-muted px-1.5 py-0.5 text-[13px] font-medium text-muted-foreground",
        className
      )}
