  ...
  return (
    <div className={cn("rounded-[18px] border border-[var(--ds-border,#eceef3)] bg-gradient-to-br from-orange-500/10 to-rose-500/5 p-4", className)}>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <div className="text-[14px] uppercase tracking-wide" style={{ color: "var(--sub,#62677E)" }}>{t("streak.title")}</div>
            ...
            >
              <Snowflake className="h-3 w-3" style={freezes > 0 ? undefined : { color: "var(--sub,#62677E)" }} />
          ...
          {longest > current && (
            <div className="text-[14px]" style={{ color: "var(--sub,#62677E)" }}>{t("streak.record", { longest })}</div>
          )}
        </div>
      </div>
      ...
      {!usedFreeze && freezes === 0 && current > 0 && (
        <p className="mt-3 rounded-[10px] p-2 text-[14px]" style={{ background: "var(--ds-surface2,#fbfbfc)", color: "var(--sub,#62677E)", border: "1px solid var(--ds-border,#eceef3)" }}>
          {t("streak.noFreeze")}
        </p>
      )}
