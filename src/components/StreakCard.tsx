  ...
  return (
    <div className={cn("rounded-[18px] border border-[#eceef3] bg-gradient-to-br from-orange-500/10 to-rose-500/5 p-4", className)}>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <div className="text-[14px] uppercase tracking-wide" style={{ color: "var(--sub,#6b7088)" }}>{t("streak.title")}</div>
          ...
          {longest > current && (
            <div className="text-[14px]" style={{ color: "var(--sub,#6b7088)" }}>{t("streak.record", { longest })}</div>
          )}
        </div>
      </div>
      ...
      {!usedFreeze && freezes === 0 && current > 0 && (
        <p className="mt-3 rounded-[10px] p-2 text-[14px]" style={{ background: "#fbfbfc", color: "var(--sub,#6b7088)", border: "1px solid #eceef3" }}>
          {t("streak.noFreeze")}
        </p>
      )}
