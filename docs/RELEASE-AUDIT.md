# oTutorHub — Release Audit (June code)

> Свіжий аудит проти **поточного прод-коду** (`origin/main`, 13 червня), а не старого знімка.
> 6 паралельних аудиторів (статичні перевірки, security/RLS, i18n, дизайн-система, 4 ролі, a11y/perf) → 56 знахідок → синтез.
> Гейти: `tsc` ✅ 0 · `check-i18n` ✅ (2469) · `check-ux` ✅ (0 помилок) · `vite build` ✅ · **`check-hardcode` ❌ (377 / ліміт 50)**.

## P0 — блокери релізу

| # | Роль | Що | Файл | Фікс |
|---|---|---|---|---|
| P0-1 ✅ | all | **`check-hardcode` гейт — ЗЕЛЕНИЙ.** Оркестрований sweep (45 агентів, по файлу) обгорнув ~719 рядків у `t()` + ключі uk/en/sv (зведено вручну, 34 колізії пропущено). **377 → 12** (ліміт 50). Усі 4 гейти зелені вперше. | `45 файлів + локалі` | ✅ зроблено. |
| ~~P0-2/3~~ ✅ | manager | **НЕ дефект — навмисно (рішення власника):** менеджер = адмін сайту й бачить усі дані. Звірено по міграціях: широкий доступ суворо `has_role(auth.uid(),'manager')`-gated (не всі залогінені). **Ізоляція репетиторів/учнів збережена:** `profiles` → relationship-scoped (блокет-політику дропнуто 20260426115625); `lessons_select` non-manager = `auth.uid()=tutor_id OR student_id OR group-member`; `student_rates` = `Tutor/Student views own`. Тобто репетитор бачить лише свої уроки/ставки, учень — лише свої; одне одного не бачать. **Supabase не знадобився.** | `migrations` | Закрито. Дрібний клінап: `20260523174110` (спроба обмежити менеджера) посилається на неіснуючу `workspace_settings` — moot, але варто прибрати. |

## P1 — важливе

| # | Роль | Що | Файл |
|---|---|---|---|
| P1-1 | manager | Менеджер читає **вкладення уроків незалежних** (метадані + приватні файли в storage) — RLS без фільтра source. | `migrations/20260511202330…:64` |
| P1-2 | all | **SchedulePage ковтає помилку** запиту уроків → порожній розклад без тоста/ретраю (не відрізнити від «немає уроків»). | `SchedulePage.tsx:403` |
| P1-3 | all | **Чат вантажить усі повідомлення** без `.limit()` і без віртуалізації — довгий тред монтує всю історію щоразу. | `ChatsPage.tsx:405` |
| P1-4 | all | **i18n-хардкод (377 рядків)** по екранах: OnboardingFlowB (86), SubscriptionPage (37, paywall), ChatsPage (17, **нова чат-фіча**), FinancesPage (28), MyStudentsPage (16), кабінет учня (платежі/домашки/дашборд/онбординг), mid-tier ~96 у 9 шеред-компонентах. en/sv бачать українську. |
| P1-5 ✅ | all | **`uk-UA` локаль** → хелпер `getLocale()` (`lib/locale.ts`) + центральний `formatPrice` + 81 сайт у 36 файлах. |
| P1-6 ✅ | all | **Хардкод-тости** виплат у `DashboardPage` → `t()`. (FinancesPage — через sweep.) |
| P1-7 ✅ | all | **Негативні empty-state** → позитивні: `DashboardPage`, `FinanceWeeklyChart`, `FinancesPage`. |
| P1-8 ✅ | all | **Ручний плюрал** виплат → i18next `_one/_few/_many`. (Решта ручних — дрібний P3-хвіст.) |
| P1-9 ✅ | all | **a11y:** мобільна навігація — `aria-label` + `aria-current`. |
| P1-10 ✅ | all | **a11y:** 14 інпутів `QuickAddStudentDialog`/`MyStudentsPage` → `aria-label`. |
| P1-11 ✅ | all | **a11y контраст** (рішення: бренд teal недоторканий, текст темний): центрально `--primary-foreground` біла→темна + **інлайн teal-кнопки** (оркестровано, 63 фліпи у 32 файлах; 43 білих-на-темному лишено). Усі teal-CTA тепер темний текст; #2BBFAA незмінний. |

**P1-3 ✅** Чат: пагінація (останні 50 + «↑ Показати раніші»), було без ліміту. **N+1 ✅** масові борг-нагадування → `Promise.all`.
**P2 ✅** негативні empty-state «Немає X» → позитивні (8 ключів ×3 локалі); `SubscriptionRequestDialog` → bottom-sheet.
**Контраст вторинного тексту ✅** `#9398b0`/`#b0b4c8` → `#6b7088` (~4.77:1) — 158 сайтів, лише `color:`.
**e2e-інфра ✅** `playwright.config` вантажить `.env.e2e`, скрипти `test:e2e`/`test:e2e:ui`, `.env.e2e` у `.gitignore`; 198 специв збираються (потрібні креди в `.env.e2e`).
**Плюрали ✅** виявлено й виправлено **реальний баг**: 16 ключів у en + 16 у sv мали `_one/_few/_many` без `_other` → англ/швед показували сирий ключ для «other»-кількостей. `_many`→`_other`.
**Лишається (опційне, не блокує):** нові e2e-специ для досягнень/чату менеджера (треба креди); прибрати мертві `_few` в en/sv (косметика); решта ручних плюралів (P3).

### Реконсиляція флагів пре-релізу (проти червневого коду)
- **#2 рантайм ✅** boot-smoke через Preview: застосунок піднімається, лендинг рендериться, teal-CTA з темним текстом видно вживу, **0 помилок у консолі**. (Автент. екрани — за логіном, треба креди.)
- **#5 search_path ✅** — **вже зроблено в проді**: усі SECURITY DEFINER-функції захищені (4 email-хелпери — через `ALTER … SET search_path` у `20260424140205`). Мій ранній флаг — false-positive (0 незахищених).
- **#6 uk-UA ✅** · **#8 бандл ✅ (неактуально, code-split)** · **#9 скасування ✅ (не в гілці)** · **color-mix ✅ (0)**
- **#7 a11y focus ✅** — глобальна `:focus-visible`-обводка (teal `--ring`) для всіх інтерактивних елементів; перекриває інлайн `outline:none` (21 сайт). Лише клавіатура, не мишка.
- **#10 аналітика ✅** — події `achievements_view` + `hub_manager_chat` у наявний `paywall_events` через `usePaywallTracking`.
- **Лишається (треба твоє):** #3 жива звірка міграції (Supabase-доступ) · #2 автент. екрани + #4 e2e-прогін (тестові креди) · org-рівень (GDPR/monitoring/rollback).

## P2 — полірування (зокрема нова фіча досягнень)

- **P2-1 (нова фіча):** ґрід досягнень — назва 10px / лічильник 9px, нижче мінімуму. `StudentAchievementsGrid.tsx:41`
- **P2-2 (нова фіча):** сторінка досягнень — shadcn `<Card rounded-lg>` (8px) + семантичні токени, тоді як інші екрани учня — inline `borderRadius:16/18` + `#eceef3`. CLAUDE.md: `rounded-[16px]`. `StudentAchievementsPage.tsx:26`
- **P2-3 (нова фіча):** empty-state досягнень «Поки що порожньо» — негативне обрамлення. `uk.ts` `studentAchievements.empty`
- **P2-4:** багато i18n empty-state значень з «Немає X». **P2-5:** `toasts.ts` хардкод-укр + у SKIP. **P2-6:** 37 `{{count}}`-ключів без плюралів. **P2-7:** `StudentLayout` тема/мова `h-9`. **P2-8:** `QuickActionsCard` поля `h-10 md:h-9`. **P2-9:** icon-кнопки без `aria-label`. **P2-10:** низький контраст inline `#9398b0/#b0b4c8`. **P2-11:** N+1 нотифікації в борг-нагадуванні. **P2-12:** `SubscriptionRequestDialog` не bottom-sheet. **P2-13:** e2e не запускається (нема `.env.e2e`), 0 покриття нових фіч.

## P3 — дрібне

Бандл 488 kB (норма) · non-constant-time порівняння CRON_SECRET · `.env` трекнутий (лише публічний anon-ключ) · сторінка досягнень не в постійній нав учня · спінер замість скелетона · хардкод-hex (значення збігаються з токенами) · надлишкові `_few/_many` у en/sv.

## Що з цього стосується наших нових фіч
Нові файли (achievements lib/hook/grid/page, RewardCollection, hub-manager RPC/міграція) — **чисті від хардкод-рядків** (підтверджено). Дотичні до фіч: P2-1/2/3 (стиль ґріда/картки/empty-state досягнень) і частково P1-4 (ChatsPage хардкод — переважно червневий код, не наші правки).

---
_Гілка `release-audit` = `origin/main` + 2 фіче-коміти. `main` недоторканий. Стара (травнева) робота — `release-audit-backup`._
