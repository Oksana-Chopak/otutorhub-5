...
| `20260909200000_db_backup_bucket_and_cron` | Бекапи (премортем п.8): приватний бакет db-backups (без жодної політики — лише service role) + pg_cron 23:45 UTC → edge db-backup (gzip-знімок 24 бізнес-таблиць, ретенція 30 днів) | **не доводиться з types.ts** (бакет і cron.job поза types). Вручну: зранку в Storage → db-backups має бути backup-YYYY-MM-DD.json.gz | ⛔ чекає Run у Lovable + деплой db-backup (перетимстемплено 09.09: файл опинився нижче водяного знаку після хешів Lovable — правило порядку з CLAUDE.md) |
...
| `20260910100000_landing_funnel_anon` | Воронка лендінгу для НЕзареєстрованих (10.09): таблиця-лічильник `landing_funnel_daily` (день × крок → hits + суми; жодного user_id, IP чи пристрою) + `log_landing_event` (SECURITY DEFINER, білий список із 5 імен, числа обрізані по стелі) для `anon`. Прямі INSERT/UPDATE відкликані в усіх — писати можна лише через функцію. Читає суперадмін (картка «Воронка лендінгу» в адмінці) | `landing_funnel_daily` Row у types; `log_landing_event` у types | ⛔ чекає Run у Lovable |
