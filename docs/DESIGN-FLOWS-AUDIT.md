...
**Потребує дій Lovable:** редеплой `lesson-reminders` (студентські deep-links).

**Свідомо відкладено:** мультивалютні підсумки profit-картки/Фінансів (#16 — продукт-рішення, як показувати суми в кількох валютах); показ штрафу за скасування у фінансах (#37 — потрібен маркер `is_cancellation_fee` у схемі); картка «запит у менеджера в роботі» на дашборді учня (#40 — UI-дизайн); DB-дедуп create_notification (#42 — мітигейтовано per-entity типами на клієнті); ManagerNotes title/perMonth дрібниці (#33); валюта в онбординг-формі (#20 — навмисна простота, UAH-дефолт).
