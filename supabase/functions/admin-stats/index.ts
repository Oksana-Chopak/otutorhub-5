...
// ── CRM (спека адмін-панелі) ──
    ...
    const [wsR, rolesR, profR, contR, lessAllR, ldR, payR, errR, refR, streakR, ratesR] = await Promise.all([
      ...
      admin.from("lessons").select("id, tutor_id, starts_at, status").gte("starts_at", new Date(now - 45 * day).toISOString()),
      ...
      admin.from("error_log").select("user_id, created_at").gte("created_at", new Date(now - 7 * day).toISOString()),
    ...
    const nameOf = new Map((profR.data ?? []).map((p: any) => [p.id, [p.first_name, p.last_name].filter(Boolean).join(" ")]));
    const contactOf = new Map((contR.data ?? []).map((c: any) => [c.user_id, { email: c.email, telegram: c.telegram }]));
    const lessons = lessAllR.data ?? [];
    const tutorOfLesson = new Map(lessons.map((l: any) => [l.id, l.tutor_id]));
    ...
    for (const p of paidPays) {
      ...
      if (!firstPaidAt.has(p.tutor_id) || t < firstPaidAt.get(p.tutor_id)!) firstPaidAt.set(p.tutor_id, t);
      if (p.period_end && (!lastPeriodEnd.has(p.tutor_id) || p.period_end > lastPeriodEnd.get(p.tutor_id)!)) lastPeriodEnd.set(p.tutor_id, p.period_end);
    }
    ...
    for (const r of ratesR.data ?? []) {
      ...
      activeStudents.get(r.tutor_id)!.add(r.student_id);
    }
    ...
    const stageOf = (w: any, act: boolean) => {
      const created = new Date(w.created_at).getTime();
      if (w.subscription_status === "past_due") return "payment_problem";
      if (w.subscription_status === "active") return "paying";
      if (ltvByTutor.has(w.user_id) && w.subscription_status !== "active") return "churned";
      if (w.trial_until && new Date(w.trial_until).getTime() > now) {
        return new Date(w.trial_until).getTime() - now <= 5 * day ? "trial_ending" : "trial";
      }
      if (!w.onboarding_completed) return now - created > 3 * day ? "stuck_onboarding" : "new";
      return act ? "activated" : "new";
    };
    const rows: any[] = [];
    for (const w of ws) {
      ...
      const lastLesson = streak ? new Date(streak).getTime() : comp.last;
      const daysSince = lastLesson ? Math.floor((now - lastLesson) / day) : null;
      const activated = comp.total >= 1;
      const stage = stageOf(w, activated);
      if (w.subscription_status === "active") { activeN++; mrr += PLAN_USD[w.current_plan ?? "pro_monthly"] ?? 7; }
      if (w.trial_until && new Date(w.trial_until).getTime() > now) trialN++;
      const fp = firstPaidAt.get(w.user_id);
      if (fp && fp >= monthStart.getTime()) newPaid++;
      if (stage === "churned") churned++;
      const trialLeft = w.trial_until ? Math.ceil((new Date(w.trial_until).getTime() - now) / day) : null;
    ...
    const riskOrder = { red: 0, orange: 1, green: 2 } as const;
    rows.sort((a, b) => riskOrder[a.risk] - riskOrder[b.risk] || (b.paid_us - a.paid_us));
    ...
    const funnel = [...cohorts.entries()].sort((a, b) => b[0].localeCompare(a[0])).slice(0, 8)
      .map(([week, c]) => ({ week, ...c }));
    const crm = { funnel, money: { active: activeN, trial: trialN, new_paid_month: newPaid, churned_month: churned, mrr_usd: Math.round(mrr * 100) / 100 }, tutors: rows };

}JSON.stringify({ crm,— PLATFORM-WIDE statistics for the superadmin (platform owner).
//
// Security: the caller is identified by their own JWT, then checked against the
// platform_admins table using the service role. Only a superadmin gets data; everyone
// else gets 403. This is the ONLY place that reads across all hubs/tutors — per-user /
