-- Fix: 20260708000000 created purge_user_data referencing public.tutor_student_notes —
-- a table that was DROPPED long ago. CREATE FUNCTION succeeds (plpgsql resolves table
-- names at CALL time, not creation), so the function exists but ERRORS the moment it runs
-- ("relation public.tutor_student_notes does not exist"), breaking the account-delete purge
-- and any manual purge_user_data() call. This re-issues the function WITHOUT the dead table
-- (high timestamp, so it actually applies over the broken one). Identical otherwise.
