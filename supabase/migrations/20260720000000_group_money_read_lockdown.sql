/* ============================================================================
   ...
   Part 1 (20260719000000) gated the WRITES. This closes the READS: a hub tutor
   could still SELECT hub students' group prices/payment status straight from
   lesson_participants and group_enrollments (their FOR ALL/SELECT policies are
   row-level only). Mirrors the 20260715 lessons_visible pattern:

   1. lesson_participants_visible — SECURITY DEFINER view exposing participant
      rows + flat parent-lesson fields, with the money columns masked per role:
      visible to hub-scoped managers, the student (own row), and the owning
      INDEPENDENT tutor; a hub tutor keeps the roster (attendance) but money
      reads NULL. Row filter replicates the table's SELECT RLS exactly.
   2. group_enrollments_visible — same for the configured per-student group
      price (price_per_lesson).
   3. BEFORE INSERT trigger snapshots student_price from group_enrollments
      server-side when the client sends NULL — so group lessons created by a hub
      tutor (who can no longer read enrollment prices) still get correct prices.
   4. Table-level SELECT REVOKE + column-scoped GRANT-back of the non-money
      columns (all app users share `authenticated`; the views are the only
      money-read path).

   Frontend readers are rerouted to the views in the same Publish.
   Idempotent. Timestamp strictly above 20260719000000.
   ============================================================================ */

/* ── (1) lesson_participants_visible ─────────────────────────────────────── */
...
/* ── (3) Server-side price snapshot on participant creation ───────────────── */
...
/* ── (4) Column-level SELECT lock on the base tables ─────────────────────── */
