...
-- Cascade purge a user (manager-only). Removes all related data and the profile itself.
...
BEGIN
  ...
  -- Storage: remove physical files for this user's lesson + chat attachments
  DELETE FROM storage.objects so
   USING public.lesson_attachments la
   WHERE so.bucket_id = 'lesson-attachments'
     AND so.name = la.storage_path
     AND (
       la.uploader_id = _user_id
       OR la.lesson_id IN (
  ...
  DELETE FROM storage.objects so
   USING public.chat_message_attachments ca
   WHERE so.bucket_id = 'chat-attachments'
     AND so.name = ca.storage_path
     AND (
       ca.uploader_id = _user_id
       OR ca.thread_id IN (
  ...
  -- Avatar (best-effort: ignore if missing)
