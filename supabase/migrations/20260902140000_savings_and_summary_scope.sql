/* ============================================================================
   Дві помилки, знайдені при аудиті одразу після застосування хвиль 02.09.

   (1) «ЗАОЩАДЖЕНО» БРЕХАЛО КОЖНОМУ — це моя власна регресія.
   Переписуючи get_referral_savings_uah під нову ціну (299 замість 249), я
   загубила фільтр по `reason`, який був у живій версії 20260716000000:
       AND reason IN ('referral_pro_upgrade','referral_3_pro_in_month',
                      'referral_signup_referrer')
   Без нього лічильник сумує ВЕСЬ pro_bonus_ledger, а туди пишуть ще
   `referral_signup_referred` (власний вітальний місяць за те, що ТЕБЕ привели)
   і `streak_30_days` (30 днів за серію). Наслідок на /my-referrals: репетитор,
   який не привів нікого, бачить поруч «Запрошено 0» і «Заощаджено 299 ₴».
   Фільтр повернуто дослівно.

   (2) ХАБОВИЙ РЕПЕТИТОР ЧИТАВ ПЛАТІЖНУ СТАТИСТИКУ ШКОЛИ.
   get_tutor_monthly_summary рахує «скільки уроків оплачено» по lesson_details
   без гейта на source. Для хабового це гроші, які учні платять ШКОЛІ, — рівно
   те, що спеціально маскує lessons_visible (20260715000000: student_price
   видно лише менеджеру, учневі та власнику НЕЗАЛЕЖНОГО уроку).
   Витоку не було з 05.05 лише тому, що RPC падав на дропнутій колонці; вчора
   його полагодили — і разом із ним ожив витік.
   Тепер грошові лічильники рахуються ЛИШЕ по source='independent'. Кількість
   проведених уроків і перцентиль лишаються для всіх: це власна робота
   репетитора, не чужі гроші.

   Те саме стосується умови бейджа «Боргів немає» — вона рахувала неоплачені
   рахунки школи, тож хабовий не міг його отримати ніколи, а самостійний без
   проставлених цін отримував безкоштовно. Гейт додано і туди.

   Timestamp вище останньої застосованої (20260902105639) — ordering trap.
   ============================================================================ */
